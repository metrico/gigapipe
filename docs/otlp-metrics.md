...
- Remaining resource attributes go to a `target_info` gauge (constant value
  1, one sample per ingested timestamp), not onto every series.
...
| ExponentialHistogram (cumulative) | converted to classic `_bucket`/`_sum`/`_count` series with exact boundaries (`base = 2^(2^-scale)`) |
