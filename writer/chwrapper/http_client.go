...
func (c *HttpChClient) getValue(data interface{}, row int) interface{} {
	...
	if _v, ok := value.(HttpFormatter); ok {
	...
	if (rv.Kind() == reflect.Slice || rv.Kind() == reflect.Array) &&
		...
		res := make([]any, rv.Len())
		for i := 0; i < rv.Len(); i++ {
			res[i] = rv.Index(i).Interface().(HttpFormatter).HttpValues()
		}
		return res
	}
	return value
...
func (c *HttpChClient) GetSetting(ctx context.Context, tp string, name string) (string, error) {
	fp := helputils.FingerprintLabelsDJBHashPrometheus([]byte(
...
func (r *sqlRowsWrapper) HasData() bool {
	return true
}
