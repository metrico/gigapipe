...
func analyzeOffset(script *parser.LogQLScript) *time.Duration {
	offset := findFirst[parser.Offset](script)
	if offset == nil {
		return nil
	}
	d, _ := offset.Duration()
...
func AnalyzeMetrics15sShortcut(script *parser.LogQLScript) bool {
	var lraOrUnwrap = findFirst[parser.LRAOrUnwrap](script)
	if lraOrUnwrap == nil {
		return false
	}
...
func (p *planner) getFunctionOrder(script any) {
	maybeComparison := func(op *parser.Comparison) {
		if op != nil {
...
func findFirst[T any](nodes ...any) *T {
	for _, n := range nodes {
		...
		case *parser.StrSelCmd:
			res = findFirst[T](&_n.Val, &_n.Label)
		case *parser.StrSelectorPipeline:
			res = findFirst[T](
				_n.Parser,
				_n.LineFilter,
				_n.Unwrap,
