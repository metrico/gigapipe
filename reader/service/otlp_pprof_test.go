...
func TestOtlpToPProfTimestampsOnly(t *testing.T) {
	profs := pprofile.NewProfiles()
	dict := profs.Dictionary()
	dict.StringTable().Append("", "samples", "count", "main")
	f0 := dict.FunctionTable().AppendEmpty()
	f0.SetNameStrindex(3)
	l0 := dict.LocationTable().AppendEmpty()
	l0.Lines().AppendEmpty().SetFunctionIndex(0)
	...
	s := p.Samples().AppendEmpty()
	s.SetStackIndex(0)
	s.TimestampsUnixNano().Append(100, 200) // no values: 2 observations
	...
	if len(out.Sample) != 1 || out.Sample[0].Value[0] != 2 {
		t.Fatalf("sample value should count timestamps: %+v", out.Sample)
	}
}
